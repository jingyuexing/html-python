...
# @Last Modified time: 2024-01-31 14:32:29
...
class Tailwind:
    ...
    def boot(self):
        ...
        def alpha_color(colors:str):
            [color_name,alpha] = colors.split("/")
            _val = accessNested(self.configure,[f"colors-{color_name}"])
            if _val is not None:
                r,g,b = hex_to_rgb(_val)
                return alpha,colorsys.rgb_to_hls(r,g,b)
        def background_color_alpha(color:str):
            color_ = alpha_color(color)
            if color_ is not None:
                alpha,hsl = color_
                h,s,l = hsl
                return [
                    f"--bg-opacity:{alpha}%;",
                    f"background-color:hsla({h},{s}%,{l}% / var(--bg-opacity));"
                ]
            return [""]
        def text_colors_alpha(color:str):
            color_ = alpha_color(color)
            if color_ is not None:
                alpha,hsl = color_
                h,s,l = hsl
                return [
                    f"--text-opacity:{alpha}%;",
                    f"color:hsla({h},{s}%,{l}% / var(--text-opacity));"
                ]
            return []
        def text_colors(color:str):
            part = color.split("/")
            if len(part) > 1:
                return text_colors_alpha(color)
            else:
                _val = accessNested(self.configure,[f"colors-{color}"])
                if _val is not None:
                    return [f"color:{_val};"]
            return []
        def background_color(color:str):
            if "/" in color:
                return background_color_alpha(color)
            else:
                _val = accessNested(self.configure,[f"colors-{color}"])
                if _val is not None:
                    return [f"background-color:{_val};"]
            return []
        def text(mode:str):
            part_mode = mode.split("-")
            if part_mode[0] in self.configure['colors'].keys():
                return text_colors(mode)
            elif part_mode[0] in self.configure['text'].keys():
                return text_size_config(mode)
            return []
        ...
        self.addRule(r"text-([\w-]+)", text)
        self.addRule(r"bg-(\w+)",background_color)
